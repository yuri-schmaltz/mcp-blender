...
def validate_port(port: Union[int, str]) -> int:
    ...
    if not 1024 <= port_int <= 65535:
        raise ValidationError(
            f"Port must be between 1024-65535 (privileged ports not allowed), "
            f"got: {port_int}"
        )
...
def validate_file_path(path: Union[str, Path], must_exist: bool = True) -> Path:
    ...
    if not path_obj.is_absolute():
        raise ValidationError(
            f"Path must be absolute to avoid unintended locations: {path}"
        )
    ...
    try:
        ...
        try:
            ...
            raise ValidationError(
                f"Path must be under home directory or temp directory: {path}"
            )
...
def validate_host(host: str) -> str:
    ...
    if not re.match(hostname_pattern, host):
        raise ValidationError(
            f"Invalid hostname: {host}. Must be valid IP or hostname."
        )

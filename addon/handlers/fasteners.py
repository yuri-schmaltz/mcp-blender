...
def generate_fastener(scene, type="SCREW", size="M3", length=10.0, head_type="SOCKET", location=(0,0,0), axis=(0,0,1)):
    ...
    try:
        ...
        if type.upper() == "SCREW":
            ...
            bpy.ops.mesh.primitive_cylinder_add(radius=shaft_dia/2, depth=length_m, location=shaft_loc)
            ...
            if h_type == "HEX":
                bpy.ops.mesh.primitive_cylinder_add(vertices=6, radius=(head_dia/math.sqrt(3)), depth=head_depth, location=head_loc)
            else:
                bpy.ops.mesh.primitive_cylinder_add(radius=head_dia/2, depth=head_depth, location=head_loc)
            ...
            bpy.ops.mesh.primitive_cylinder_add(vertices=6, radius=radius_circ, depth=thickness, location=location)
            ...
            bpy.ops.mesh.primitive_cylinder_add(radius=inner_dia/2, depth=thickness * 1.5, location=location)
            ...
            bpy.ops.mesh.primitive_cylinder_add(radius=outer_dia/2, depth=thickness, location=location)
            ...
            # Outer ring
            bpy.ops.mesh.primitive_cylinder_add(radius=outer_dia/2, depth=width, location=location)
            ...
            # Inner hole cutter
            bpy.ops.mesh.primitive_cylinder_add(radius=inner_dia/2, depth=width * 1.5, location=location)
            ...
            shield_r_min = inner_dia * 0.6 + outer_dia * 0.4
            shield_r_max = inner_dia * 0.4 + outer_dia * 0.6

...
def extrude_faces(scene, name, amount=1.0, select_all=True):
    ...
    try:
        ...
        bpy.ops.mesh.extrude_region_shrink_fatten(TRANSFORM_OT_shrink_fatten={"value": amount})
...
def inset_faces(scene, name, thickness=0.1, depth=0.0, select_all=True):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Inset faces of '{name}' (thickness={thickness}, depth={depth}).",
        }
    except Exception as e:
        bpy.ops.object.mode_set(mode="OBJECT")
        return {"error": str(e)}
...
def bevel_edges(scene, name, width=0.1, segments=1, profile=0.5, select_all=True):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Beveled edges of '{name}' (width={width}, segments={segments}).",
        }
    except Exception as e:
        bpy.ops.object.mode_set(mode="OBJECT")
        return {"error": str(e)}
...
def subdivide_mesh(scene, name, number_cuts=1, smoothness=0.0):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Subdivided '{name}' ({number_cuts} cuts). Now {verts} verts, {faces} faces.",
        }
    except Exception as e:
        bpy.ops.object.mode_set(mode="OBJECT")
        return {"error": str(e)}
...
def merge_vertices(scene, name, threshold=0.0001):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Merged vertices on '{name}'. Removed {removed} duplicates (threshold={threshold}).",
        }
    except Exception as e:
        bpy.ops.object.mode_set(mode="OBJECT")
        return {"error": str(e)}
...
def spin_mesh(scene, name, axis="Z", angle=360, steps=32):
    ...
    try:
        ...
        bpy.ops.mesh.spin(
            steps=steps, angle=math.radians(angle), axis=axis_vec, center=obj.location
        )
        bpy.ops.object.mode_set(mode="OBJECT")

        return {
            "success": True,
            "message": f"Spun '{name}' {angle}° around {axis} in {steps} steps.",
        }
    except Exception as e:
        bpy.ops.object.mode_set(mode="OBJECT")
        return {"error": str(e)}
...
def recalculate_normals(scene, name, inside=False):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Recalculated normals of '{name}' to face {direction}.",
        }
    except Exception as e:
        bpy.ops.object.mode_set(mode="OBJECT")
        return {"error": str(e)}

...
def handle_chat_request_headless(
    prompt, provider, model, api_key, allow_code_execution, base_url=None, execution_callback=None
):
    """
    ...
    if litellm is None:
        return {
            "error": "litellm is not installed. Please run 'Check/Install Dependencies' in the panel."
        }
    ...
    if profile != "ALL":
        ...
        if profile == "MODELING":
            ...
            allowed_tools.update({"mark_as_functional_part", "list_functional_parts"})
        ...
        print(
            f"BlenderMCP Embedded Chat: Filtered tools count down to {len(tools_list)} for profile {profile}"
        )
    ...
    try:
        ...
        if response_message.tool_calls:
            ...
            for tool_call in response_message.tool_calls:
                ...
                tool_command = {"type": function_name, "params": function_args}
            ...
            if getattr(final_message_obj, "refusal", None):
                return {
                    "status": "error",
                    "message": f"Request refused: {final_message_obj.refusal}",
                }
            ...
            if getattr(response_message, "refusal", None):
                return {
                    "status": "error",
                    "message": f"Request refused: {response_message.refusal}",
                }
            ...
            if matches and allow_code_execution:
                ...
                return {
                    "status": "success",
                    "message": "Python code extracted and executed.\n" + content,
                }
...
def handle_chat_request(context):
    ...
    if provider == "OLLAMA":
        model = (
            prefs.llm_model_custom if prefs.llm_model_ollama == "MANUAL" else prefs.llm_model_ollama
        )
    else:
        model = (
            prefs.llm_model_custom
            if prefs.llm_model_custom_enum == "MANUAL"
            else prefs.llm_model_custom_enum
        )

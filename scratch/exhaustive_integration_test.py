...
def run_tests():
    ...
    try:
        ...
        assert "Cylinder" in bpy.context.scene.objects, (
            "Cylinder object was not created in Blender!"
        )
        print("-> Test 2 PASSED!\n")
        ...
        assert any("Washer" in name or "washer" in name.lower() for name in object_names), (
            "Washer object was not created in Blender!"
        )
        print("-> Test 3 PASSED!\n")
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            assert any("Cylinder" in name for name in web_object_names), (
                "Cylinder not found in WebUI API response"
            )
            assert any("Washer" in name or "washer" in name.lower() for name in web_object_names), (
                "Washer not found in WebUI API response"
            )
            print("-> Test 5 PASSED!\n")

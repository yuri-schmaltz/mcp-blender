...
def test_analyze_viewport_visuals_success(tmp_path):
    ...
    def mock_get_screenshot(scene, max_size, filepath, format):
        return {"success": True, "width": 800, "height": 600, "image_base64": base64.b64encode(b"dummy image data").decode("utf-8")}
    ...
    with patch("addon.handlers.visual_critic.get_viewport_screenshot", side_effect=mock_get_screenshot), \
...
def test_analyze_viewport_visuals_model_not_found(tmp_path):
    def mock_get_screenshot(scene, max_size, filepath, format):
        return {"success": True, "image_base64": base64.b64encode(b"dummy image data").decode("utf-8")}

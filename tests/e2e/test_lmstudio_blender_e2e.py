...
def run_tests():
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a professional 3D automation agent connected to Blender via MCP. Your goal is to fulfill the user's prompt by calling the provided tools sequentially.",
        },
        {
            "role": "user",
            "content": "Adicione uma malha de Suzanne (macaco) chamada 'SuzanneAgent', pinte com um material de ouro puro (cor amarela, 100% metálico, 0% rugosidade). Depois, configure uma iluminação de 3 pontos focada nela e por fim adicione uma animação turntable de 100 frames. Use as ferramentas adequadas.",
        },
    ]
    ...
    while steps < max_steps:
        ...
        response = client.chat.completions.create(
            model="local-model", messages=messages, tools=openai_tools, temperature=0.1
        )
        ...
        for tool_call in message.tool_calls:
            ...
            # Forward to MCP
            mcp_res = send_request("tools/call", {"name": fn_name, "arguments": fn_args})
